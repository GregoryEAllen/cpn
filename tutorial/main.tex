...
the node through the \newline\verb!VariantCPNLoader!. The second way is by asking the kernel to create a function node.
...
In our example we shall use the \verb!Variant! class and the JSON parser to tern the
...
The first thing the constructor does it pass its parameters on to \verb!CPN::NodeBase! so that
...
The \newline\verb!CPN::QueueReaderAdapter<T>! and \verb!CPN::QueueWriterAdapter<T>! provide
...
Note that the \newline\verb!QueueReaderAdapter<T>::Dequeue(T *, unsigned)! function will return
...
There is an example application in the \url{apps/XML-JSON} which reads in JSON or XML and then outputs JSON or XML.
...
is a key value map at the base. There are four keys that specify options for the KernelAttr and three
that specify how to load the PN.
The three options for the KernelAttr are:
\begin{description}
...
\item[``database''] A sub-map that describes options for what kind of database to load and what options on the database to set. Those options are:
\begin{description}
\item[``host''] What host name the database daemon is listening on (optional, if present ``port'' must also be specified, else the local database implementation will be used).
\item[``port''] What port number or name the database daemon is listening on.
...
\item[``libs''] A list of shared libraries that contain node definitions that the database should load up immediately.
...
\item[``loglevel''] Specify the logging level for the internal CPN logger.
\end{description}
\end{description}
...
Next is an example of the Fibonacci number generator using the distributed features of CPN.
The summing and delay node are the same and are symbolic links in the code directory.
This example also uses the \verb!VariantCPNLoader! to load the PN.

A special note that the listing in this section will omit much of the error checking.
Full error checking is included in the full listing.
...
Then it loads in the file \verb!db_def.json! for the default database options.
...
\begin{lstlisting}{}
    ...
    kernel.WaitForAllNodeEnd();
\end{lstlisting}
Notice that the kernels not reading out the configuration wait for the result pseudo node to terminate.
