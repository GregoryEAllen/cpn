...
The VariantCPNLoader has a validator that goes through the configuration and ensures that all required fields are present
...
This example also uses the VariantCPNLoader to load the PN.
...
The configuration is a key value map at the base. There are four keys that specify options for the KernelAttr and three
...
Some valid types are ``void'', and the various types names defined in ``stdint.h'' like ``int32_t'' as well as ``float'', ``double'', and ``complex<T>'' where ``T'' is any of the previous types.
