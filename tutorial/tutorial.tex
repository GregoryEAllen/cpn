...
This tutorial provides an introduction using the Computational Process Network (CPN) library.
...
being thrown (unless SwallowBrokenQueueExceptions is set as a kernel parameter).
...
The data contents that requires 
...
When when creating nodes in the CPN program, the kernel which should instantiate that node is specified with \verb!NodeAttr::SetKernel!.
...
The \verb!VariantCPNLoader! to load and execute the example program specification.
...
Because the configuration is split between multiple files, parts of the configuration to be reused and combined in different ways.
...
which can be accomplished by changing only mapping file.
