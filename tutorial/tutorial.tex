...
The cons node first outputs its initial value, then copies its input to the outputs.
...
that specify how to load the PN. The specification of what it wants to see is given in section \ref{appendix b}.
...
If not then there are several places the node loader will look to see if it can find one.
...
for that node type.
You can manually add a factory to the node loader by the kernel member method \verb|RegisterNodeFactory|.
...
The problem is linker will prune out object files in archives with no references.
...
The methods of interest are \verb!NodeAttr::SetParam!, \verb!NodeAttr::GetParam! and \verb!NodeAttr::SetKernel!.
The \verb!NodeAttr::SetKernel! is used when distributing the process network.
See section \ref{sec:distributing}.
...
The \verb!NodeAttr::SetParam! method allow the passing of a
string parameters to the node.
Notice that we cannot call \verb|GetIQueue| or \verb|GetOQueue| in the constructor because
...
the queue can be created. Attribute objects can be reused.
...
\begin{lstlisting}{}
    ...
    kernel.WaitForAllNodes();
\end{lstlisting}
Notice that the kernels not reading out the result wait for the result node to terminate.
This ensures that the other kernels will stick around until we are done with the PN.
We also could have called the \verb!Kernel::Wait()! function on the kernel and in the kernel that loaded the configuration
