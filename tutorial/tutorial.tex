...
The cons node has an initial value, an innput, and two outputs.
...
Asking for a maximum number any larger than this will result in overflow and incorrect results.
...
See section \ref{sec:node loader} for more details.
...
Let us look at the configuration files in Listings \ref{lst:pn_def} and \ref{lst:nodemap}.
...
Then the command line options are parsed. Several of the command line options are not optional.
...
Notice that any configuration files that should be loaded are done so when parsing the command line options.
...
\lstinputlisting[caption=The Cons Node,label=lst:delay]{code/derived_node/Fib_Cons.cc}
